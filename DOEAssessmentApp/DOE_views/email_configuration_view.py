            ...
            if 'empid' in session and Companyuserdetails.query.filter_by(empemail=resp).first() is not None:
                ...
                if request.method == "POST":
                    ...
                    if companyemailconfcount == 0:
                        ...
                        db.session.commit()
                        # end region
                        return make_response(jsonify({"msg": f"Email Configuration successfully inserted "
                                                                 f"for your company",
                                                      "data": result[0]})), 201
                    else:
                        return make_response(jsonify({"msg": f"Email Configuration already exists for "
                                                                 f"your company"})), 400
            else:
            ...
            if 'empid' in session and Companyuserdetails.query.filter_by(empemail=resp).first() is not None:
                ...
                if data.first() is None:
                    ...
                    if request.method == 'POST':
                        ...
                        db.session.commit()
                        # end region
                        return make_response(jsonify({"msg": f"Email Configuration successfully updated "
                                                                 f"for your company",
                                                      "data": result[0]})), 200
            else:
                return make_response(jsonify({"msg": resp})), 401
